...
(function() {
  ...
  IE7.CSS.extend({
    ...
    reset: function() {
      ...
      for (var i in elements) removeStyle(elements[i]);
      this.elements = {};
      // reset runtimeStyle here
      var elements = IE7.Rule.elements;
      for (var i in elements) removeClasses(elements[i]);
      IE7.Rule.elements = {};
    },
  ...
  IE7.HTML.extend({
    ...
    addRecalc: function(selector, handler) {
      ...
      this.base(selector, function(element) {
        if (!IE7.HTML.elements[element.uniqueID]) {
          // execute the original recalc method
          handler(element);
          // store a reference to this element so that
          //  it is not "fixed" again
          IE7.HTML.elements[element.uniqueID] = element;
        }
      });
    }
  });
